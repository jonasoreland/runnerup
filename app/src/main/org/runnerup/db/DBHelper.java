...
public class DBHelper extends SQLiteOpenHelper implements Constants {
  ...
  private static final String CREATE_TABLE_ACCOUNT =
      "create table "
          ...
          + (DB.ACCOUNT.FLAGS
              ...
              + DB.ACCOUNT.DEFAULT_FLAGS
              + ", ") // Mostly not used but dynamic changes could be stored here
  ...
  private static final String CREATE_TABLE_REPORT =
      "create table "
          ...
          + " integer not null, "
          + DB.EXPORT.STATUS
          + " text, " // status for external id
          + DB.EXPORT.EXTERNAL_ID
          + " text, "
          + DB.EXPORT.EXTRA
          + " integer not null default 1" // DBVERSION update: remove
          + ");";

  private static final String CREATE_TABLE_AUDIO_SCHEMES =
      "create table "
  ...
  public void onUpgrade(SQLiteDatabase arg0, int oldVersion, int newVersion) {
    ...
    if (oldVersion < 24) {
      ...
      echoDo(
          arg0,
          "alter table " + DB.ACTIVITY.TABLE + " add column " + DB.ACTIVITY.AVG_HR + " integer");
      echoDo(
          arg0,
          "alter table " + DB.ACTIVITY.TABLE + " add column " + DB.ACTIVITY.MAX_HR + " integer");
    }

    if (oldVersion < 25) {
      echoDo(arg0, "alter table " + DB.LAP.TABLE + " add column " + DB.LAP.AVG_CADENCE + " real");
      echoDo(
          arg0,
          "alter table " + DB.LOCATION.TABLE + " add column " + DB.LOCATION.CADENCE + " real");
      echoDo(
          arg0,
          "alter table " + DB.ACTIVITY.TABLE + " add column " + DB.ACTIVITY.AVG_CADENCE + " real");
    }
    ...
    if (oldVersion < 28) {
      echoDo(
          arg0,
          "alter table " + DB.ACCOUNT.TABLE + " add column " + DB.ACCOUNT.AUTH_NOTICE + " integer");
    }

    if (oldVersion < 31) {
      echoDo(
          arg0,
          "alter table " + DB.LOCATION.TABLE + " add column " + DB.LOCATION.TEMPERATURE + " real");
      echoDo(
          arg0,
          "alter table " + DB.LOCATION.TABLE + " add column " + DB.LOCATION.PRESSURE + " real");
      echoDo(
          arg0,
          "alter table " + DB.LOCATION.TABLE + " add column " + DB.LOCATION.GPS_ALTITUDE + " real");
      echoDo(
          arg0,
          "alter table "
              + DB.LOCATION.TABLE
      ...
      echoDo(
          arg0,
          "alter table " + DB.LOCATION.TABLE + " add column " + DB.LOCATION.ELAPSED + " real");
      echoDo(
          arg0,
          "alter table " + DB.LOCATION.TABLE + " add column " + DB.LOCATION.DISTANCE + " real");
      echoDo(
          arg0,
  ...
  private void migrateFileSynchronizerInfo(SQLiteDatabase arg0) {
    ...
    Cursor c =
        arg0.query(
            DB.ACCOUNT.TABLE,
            from,
            DB.ACCOUNT.NAME + " = ? and " + DB.ACCOUNT.AUTH_CONFIG + " is not null",
            args,
            null,
            null,
            null);
  ...
  private static void insertAccounts(SQLiteDatabase arg0) {
    ...
    // "Minor changes" like adding a new syncher can be handled with updating
    // DB.DBINFO.ACCOUNT_VERSION
  ...
  private static void insertAccount(SQLiteDatabase arg0, String name, int enabled, int flags) {
    ...
    long newId =
        arg0.insertWithOnConflict(DB.ACCOUNT.TABLE, null, arg1, SQLiteDatabase.CONFLICT_IGNORE);
  ...
  public static ContentValues get(Cursor c) {
    if (c.isClosed() || c.isAfterLast() || c.isBeforeFirst()) return null;
