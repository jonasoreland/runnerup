...
public abstract class DefaultSynchronizer implements Synchronizer {
  ...
  private Status persistActivity(SQLiteDatabase db, ActivityEntity activity) {
    ...
    if (activity.getSport() == null
        ...
        || activity.getTime() == null
        || activity.getDistance() == null) {
      return Status.ERROR;
    }
    ...
    if (DBHelper.bulkInsert(activity.getLocationPoints(), db)
        != activity.getLocationPoints().size()) {
      db.endTransaction();
      return Status.ERROR;
    }
  ...
  void addCookies(HttpURLConnection conn) {
    ...
    for (String cookie : cookies) {
      if (!first) buf.append("; ").append(cookie.split(";", 2)[0]);
      first = false;
    }
  ...
  void getCookies(HttpURLConnection conn) {
     ...
     */
    if (connCookies == null) connCookies = conn.getHeaderFields().get("set-cookie");
  ...
  static String noNullStr(String s) {
