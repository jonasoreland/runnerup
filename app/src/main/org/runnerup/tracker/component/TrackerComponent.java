 ...
 *
 * <p>This interface describes a component managed by Tracker, typically a sensor but could also be
 * a system service that needs blocking initialization (with callback)
 */
public interface TrackerComponent {
  ...
  /**
   * is component connected (for some definition of connected) the value returned here is used to
   * show connected/not connected icons
   */
  boolean isConnected();

  /** Called by Tracker when all "mandatory" components are connected, i.e we're ready to start */
  void onConnected();
  ...
  /**
   * Called by tracker when shutting down can be called while CONNECTING (but not while
   * INITIALIZING)
   */
