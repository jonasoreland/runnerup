 ...
 *
 * <p>Class for managing a set of TrackerComponents as if they were one
 */
public class TrackerComponentCollection implements TrackerComponent {
  ...
  public TrackerComponent addComponent(TrackerComponent component) {
    components.put(component.getName(), new Pair<>(component, ResultCode.RESULT_OK));
    return component;
  }

  public TrackerComponent getComponent(String key) {
    synchronized (components) {
      if (components.containsKey(key)) return components.get(key).first;
      else if (pending.containsKey(key)) return pending.get(key);
      return null;
    }
  }
  ...
  public ResultCode getResultCode(String key) {
    synchronized (components) {
      if (components.containsKey(key)) return components.get(key).second;
      else if (pending.containsKey(key)) return ResultCode.RESULT_PENDING;
  ...
  public ResultCode onInit(final Callback callback, Context context) {
    return forEach(
        "onInit",
        (comp0, currentResultCode, callback0, context0) -> {
          if (currentResultCode == ResultCode.RESULT_OK) return comp0.onInit(callback0, context0);
          else return currentResultCode;
        },
        callback,
        context);
  }
  ...
  private ResultCode forEach(
      final String msg, final Func1 func, final Callback callback, Context context) {
    synchronized (components) {
      HashMap<String, Pair<TrackerComponent, ResultCode>> list = new HashMap<>(components);
      components.clear();
