...
public class Formatter implements OnSharedPreferenceChangeListener {
  ...
  private String cueElapsedTime(long seconds, boolean includeDimension) {
    ...
    if (unitCue) {
      ...
      if (seconds > 0) {
        if (hours > 0 || minutes > 0) s.append(" ");

        if (includeDimension) {
          s.append(
              cueResources.getQuantityString(R.plurals.cue_second, (int) seconds, (int) seconds));
        } else {
          s.append(seconds);
        }
      }
      ...
      if (hours > 0) {
        ...
        String sec = String.format(cueResources.audioLocale, "%02d", seconds);
        s.append(minutes).append(" ").append(sec);
      } else {
        s.append(seconds);
      }
    }
  ...
  private String txtElapsedTime(long seconds) {
    ...
    if (hours > 0) {
      s.append(hours)
          .append(" ")
          .append(resources.getString(org.runnerup.common.R.string.metrics_elapsed_h));
    }
    if (minutes > 0) {
      if (hours > 0) s.append(" ");
      if (hours > 0 || seconds > 0)
        s.append(minutes)
            .append(" ")
            .append(resources.getString(org.runnerup.common.R.string.metrics_elapsed_m));
      else
        s.append(minutes)
            .append(" ")
            .append(resources.getString(org.runnerup.common.R.string.metrics_elapsed_min));
    }
    if (seconds > 0) {
      if (hours > 0 || minutes > 0) s.append(" ");
      s.append(seconds)
  ...
  private String cuePace(double meters_per_second) {
    ...
    if (unitCue) {
      if (minutes_per_unit > 0) {
        s.append(
            cueResources.getQuantityString(
                R.plurals.cue_minute, minutes_per_unit, minutes_per_unit));
      }
      if (seconds_per_unit > 0) {
        if (minutes_per_unit > 0) s.append(" ");
        s.append(
            cueResources.getQuantityString(
                R.plurals.cue_second, seconds_per_unit, seconds_per_unit));
      }
      s.append(" ")
          .append(
              cueResources.getString(metric ? R.string.cue_perkilometer : R.string.cue_permile));
    } else {
      String secs = String.format(cueResources.audioLocale, "%02d", seconds_per_unit);
      s.append(minutes_per_unit).append(" ").append(secs);
    }
    return s.toString();
  }
  ...
  private String cueSpeed(double meters_per_second) {
    ...
    if (unitCue) {
      return cueResources.getQuantityString(
          metric ? R.plurals.cue_kilometers_per_hour : R.plurals.cue_miles_per_hour,
          (int) distance_per_hour,
          str);
    } else {
      return str;
    }
    ...
    if (meters >= base_meters * 0.99) {
      ...
      if (txt) {
        ...
        if (unitCue) {
          res =
              cueResources.getQuantityString(
                  metric ? R.plurals.cue_kilometer : R.plurals.cue_mile, (int) val, val2);
        } else {
          res = val2;
        }
      }
      ...
      if (txt) {
        res =
            String.format(
                ...
                "%d %s",
                meters,
                resources.getString(org.runnerup.common.R.string.metrics_distance_m));
      } else {
        if (unitCue) {
  ...
  public String formatName(String first, String last) {
    ...
    else if (first == null && last != null) return last;
    else if (first != null /*&& last == null*/) return first;
    return "";
  }
