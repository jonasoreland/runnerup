...
public class HRZoneCalculator {
  ...
  public Pair<Integer, Integer> getZoneLimits(int zone) {
    ...
    if (zone >= zoneLimitsPct.length) return null;

    if (zone + 1 < zoneLimitsPct.length)
      return new Pair<>(zoneLimitsPct[zone], zoneLimitsPct[zone + 1]);
    else return new Pair<>(zoneLimitsPct[zone], 100);
  }
  ...
  public Pair<Integer, Integer> computeHRZone(int zone, int maxHR) {
    ...
    return new Pair<>(
        (int) Math.round(limits.first * maxHR / 100.0),
