...
public class HRZones {
  ...
  public void reload() {
    ...
    if (zones != null) {
      StringBuilder s = new StringBuilder("loaded: (" + str + ")");
      for (int zone : zones) {
  ...
  public int getCount() {
    if (zones != null) return zones.length - 1;
    return 0;
  }
  ...
  public void save(Vector<Integer> vals) {
    ...
    for (int i = 0; i < zones.length; i++) zones[i] = vals.get(i);
