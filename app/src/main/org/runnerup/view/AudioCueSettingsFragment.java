...
public class AudioCueSettingsFragment extends PreferenceFragmentCompat {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        // Inflate our custom layout for the audio cue settings (a TitleSpinner above the list of preferences)
    ...
    private void updateSortOrder(String name) {
        mDB.execSQL("UPDATE " + Constants.DB.AUDIO_SCHEMES.TABLE + " set " + Constants.DB.AUDIO_SCHEMES.SORT_ORDER +
                " = (SELECT MAX(" + Constants.DB.AUDIO_SCHEMES.SORT_ORDER + ") + 1 FROM "
                + Constants.DB.AUDIO_SCHEMES.TABLE + ") " +
                " WHERE " + Constants.DB.AUDIO_SCHEMES.NAME + " = '" + name + "'");
    }
    ...
    private final OnPreferenceClickListener onTestCueinfoClick = new OnPreferenceClickListener() {
        ...
        private final TextToSpeech.OnInitListener mTTSOnInitListener = status -> {
            ...
            if (settingsName == null || settingsName.contentEquals(DEFAULT))
                ...
                prefs = requireContext().getSharedPreferences(settingsName + SUFFIX,
