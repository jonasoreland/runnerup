...
/**
 * Provides the {@link Fragment} instances displayed in the app's main bottom navigation.
 * This adapter is used with a ViewPager2 to manage fragment states.
 */
public class BottomNavFragmentStateAdapter extends FragmentStateAdapter {
    ...
    /**
     * Drawable resource IDs for the icons displayed in the bottom navigation tabs.
     * The order of icons in this array must match the order of fragments
     * returned by the {@link #createFragment(int)} method at the corresponding positions.
     */
    @DrawableRes
     ...
     * <p>
     * This position corresponds to the index of the tab in the bottom navigation.
     *
    ...
    public @DrawableRes int getIcon(int position) {
        if (position < 0 || position >= ICONS.length) {
            throw new ArrayIndexOutOfBoundsException("Position " + position
                    + " is out of bounds of the icons array of size " + ICONS.length);
        }
