...
public class NumberPicker extends LinearLayout {
  ...
  private void createValueText(Context context) {
    ...
    valueText.setOnFocusChangeListener(
        (v, hasFocus) -> {
          if (hasFocus) {
            valueText.selectAll();
          } else {
  ...
  private void setValueImpl(int newValue) {
    if (newValue < minValue) {
      if (wrapValue) newValue = maxValue;
      else newValue = minValue;
    } else if (newValue > maxValue) {
      if (wrapValue) newValue = minValue;
      else newValue = maxValue;
    }
    int save = prevValue;
    prevValue = currValue;
    currValue = newValue;
    if (listener != null) listener.onChanged(this, save, newValue);
    updateView();
  }
  ...
  private final OnClickListener buttonClick =
      new OnClickListener() {
        @Override
  ...
  private final OnLongClickListener buttonLongClick =
      new OnLongClickListener() {
        @Override
        public boolean onLongClick(View v) {
          if (v == incButton) buttonLongClick(+1);
          else buttonLongClick(-1);
          return true;
        }
      };

  private final OnTouchListener buttonLongTouchListener =
      new OnTouchListener() {
        @Override
