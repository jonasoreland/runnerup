...
public class SpinnerPresenter {
  ...
  private void setupEditText(
      final Context context, final AttributeSet attrs, TypedArray arr, CharSequence defaultValue) {
    mInputType =
        arr.getInt(
  ...
  private void setupDatePicker(
      final Context context, AttributeSet attrs, CharSequence defaultValue) {
  ...
  private void setupDurationPicker(
      final Context context, final AttributeSet attrs, CharSequence defaultValue) {
    setValueWithoutSave(defaultValue);
    ...
    if (mType == Type.TS_DISTANCEPICKER && !TextUtils.isEmpty(value)) {
      ...
      mSpin.setViewText(
          String.format(
              "%s %s",
              value,
              mContext.getResources().getString(org.runnerup.common.R.string.metrics_distance_m)));
    } else {
