...
public class Step implements TickComponent {
  ...
  /** DurationType Dimension.TIME, Dimension.DISTANCE, null (keypress) */
  ...
  private boolean exceedDistance(
      double distance, double prevDistance, double refDist, double time) {
  ...
  public double getTime(Workout w, Scope s) {
    ...
    if (org.runnerup.BuildConfig.DEBUG) {
      throw new AssertionError();
    }
    return 0;
  }
  ...
  public double getDuration(Dimension dimension) {
    if (durationType == dimension) return durationValue;
    return 0;
  }

  static Step createRestStep(Dimension dim, double duration, boolean recovery) {
    Step step;
    if (!recovery && (dim == null || dim == Dimension.TIME)) step = new PauseStep();
    else step = new Step();
