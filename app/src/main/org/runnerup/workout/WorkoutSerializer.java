...
public class WorkoutSerializer {
  ...
  private static RepeatStep findRepeatStep(ArrayList<jsonstep> list, int groupId) {
    for (jsonstep s : list) {
      if (s.group != null && s.group == groupId && s.step instanceof RepeatStep) {
  ...
  private static final Pair<Dimension, Double> NullDimensionPair = new Pair<>(null, 0.0);
  ...
  private static void putDuration(
      JSONObject obj, Step step, Dimension durationType, double durationValue)
      throws JSONException {
  ...
  private static double scale(double val, JSONObject obj, String key) {
    ...
    if (unit.equalsIgnoreCase("dimensionless")) return val;

    /*
     * The below are one I found in the json objects...
     */
    if (unit.equalsIgnoreCase("centimeter")) return val / 100.0;
    if (unit.equalsIgnoreCase("ms")) return val / 1000.0;
    if (unit.equalsIgnoreCase("kilojoule")) return val;
    if (unit.equalsIgnoreCase("bpm")) return val;

    /*
     * The below I just added for "completeness"
     */
    if (unit.equalsIgnoreCase("millimeter")) return val / 1000.0;
    if (unit.equalsIgnoreCase("kilometer")) return val * 1000.0;
    if (unit.equalsIgnoreCase("miles")) return val * 1609.34;
  ...
  private static Pair<Dimension, Range> getTarget(JSONObject obj) {
    ...
    if (targetTypeKey == null) return NullTargetPair;
    if (targetTypeKey.equalsIgnoreCase("no.target")) return NullTargetPair;
  ...
  private static void putTarget(JSONObject obj, Step step, Dimension targetType, Range targetValue)
      throws JSONException {
  ...
  private static void scale(Range range, Dimension dim, JSONObject obj, String key) {
    ...
    if (unit.equalsIgnoreCase("dimensionless")) return;
