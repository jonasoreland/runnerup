...
public class AntPlus extends BtHRBase {
  ...
  private final IAsyncScanResultReceiver scanReceiver =
      new IAsyncScanResultReceiver() {
        ...
        public void onSearchResult(final AsyncScanResultDeviceInfo arg0) {
          ...
          final HRDeviceRef ref =
              HRDeviceRef.create(
                  NAME, arg0.getDeviceDisplayName(), Integer.toString(arg0.getAntDeviceNumber()));
          ...
          if ((mIsConnecting || mIsConnected)
            ...
            releaseHandle =
                AntPlusHeartRatePcc.requestAccess(
                    context, arg0.getAntDeviceNumber(), 0, resultReceiver, stateReceiver);
            return;
          }
          ...
          hrClientHandler.post(
              () -> {
                if (mIsScanning) { // NOTE: mIsScanning in user-thread
                  hrClient.onScanResult(
                      HRDeviceRef.create(
                          NAME,
                          arg0.getDeviceDisplayName(),
  ...
  private void reportDisconnected(final boolean b) {
    ...
    hrClientHandler.post(
        () -> {
          if (hrClient != null) hrClient.onDisconnectResult(b);
        });
  }
