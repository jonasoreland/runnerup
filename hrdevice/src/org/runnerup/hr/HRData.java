 ...
 *
 * <p>Contains: * heart rate (as calculated on the device * time stamp (either from the device, or
 * an estimate) * R-R interval
 *
 * <p>Depending on the API provided by the protocol module, it's possible that not all fields are
 * available. Because of this, each datum has a corresponding flag that determines whether the value
 * has been set or not.
 *
 * <p>These need to be checked before using the data. For example
 *
 * <pre>{@code
...
public class HRData {
  ...
  public String toString() {
    return "HRData{"
        + ", hrValue="
        + (hasHeartRate ? hrValue : "<no_heart_rate_data>")
        + '\n'
        ...
        + '\n'
        + ", rrIntervals="
        + (hasRrIntervals ? Arrays.toString(rrIntervals) : "<no_rr_interval_data>")
        + '\n'
        + '}';
  }
