...
public class HRManager {
   ...
   * @return A new instance of an {@link HRProvider} or null if A) 'src' is not a valid {@link
   *     HRProvider} type B) the device does not support an {@link HRProvider} of type 'src'
   */
  ...
  private static HRProvider getHRProviderImpl(Context ctx, String src) {
    ...
    if (src.contentEquals(Bt20Base.ZephyrHRM.NAME)) {
      if (!Bt20Base.checkLibrary(ctx)) return null;
    ...
    if (src.contentEquals(AntPlusProxy.Name)) {
      if (!AntPlusProxy.checkAntPlusLibrary(ctx)) return null;
   ...
   *
   * <p>It is recommended to use this list only for selecting a valid {@link HRProvider}. For
   * connecting to the device, use the instance returned by {@link
   * #getHRProvider(android.content.Context, String)}
   *
