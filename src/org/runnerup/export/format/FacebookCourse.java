...
public class FacebookCourse {
	...
	private JSONArray trail(long activityId) throws JSONException {
		final String cols[] = { DB.LOCATION.TYPE, DB.LOCATION.LATITUDE,
		...
		if (c.moveToFirst()) {
			...
			do {
				...
				case DB.LOCATION.TYPE_RESUME:
					...
					accTime = period * 1000; // always emit first point start/resume
					break;
	...
	private JSONObject point(Location prev, Location last, long sumTime, double sumDist) throws JSONException {
		JSONObject obj = new JSONObject();
		obj.put("location", new JSONObject().put("latitude", last.getLatitude()).put("longitude", last.getLongitude()));
