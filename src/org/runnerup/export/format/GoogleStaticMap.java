...
public class GoogleStaticMap {
	...
	public static void encode(StringBuffer dst, long latitude1, long longitude1, long latitude0, long longitude0) {
	...
	public String export(long activityId, final int maxLen) {
		...
		do {
			...
			String[] args = { Long.toString(activityId) };
			Cursor c = mDB.rawQuery("SELECT DISTINCT cast(round(" + DB.LOCATION.LATITUDE + "*100000) as integer), cast(round(" + DB.LOCATION.LONGITUDE + "*100000) as integer) FROM " + DB.LOCATION.TABLE + " WHERE " + DB.LOCATION.ACTIVITY + " = ?", args);
			if (c.moveToFirst()) {
				long lat0 = 0;
				long long0 = 0;
				do {
