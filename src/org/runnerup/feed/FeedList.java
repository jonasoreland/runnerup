...
public class FeedList extends Observable implements Constants {
	...
	public void load() {
		...
		Cursor c = db.query(DB.FEED.TABLE, null, null, null, null, null, DB.FEED.START_TIME + " desc", Integer.toString(MAX_ITEMS));
		if (c.moveToFirst()) {
			do {
	...
	public void prune() {
		if (list.size() >= MAX_ITEMS + 1) {
			...
			db.execSQL("DELETE FROM " + DB.FEED.TABLE + " WHERE " + DB.FEED.START_TIME + " < " + start_time);
			db.close();
	...
	public class FeedUpdater {
		...
		FeedUpdater() {
			...
			for (ContentValues c : list) { // initialize list with already present items
				if (!isHeaderDate(c))
					currList.add(c);
			}
		...
		public void complete() {
			...
			mDB = null;
			prune();
			System.err.println("FeedUpdater: " + uploader + ", added: " + added + ", discarded: " + discarded);
		}
	...
	public static List<ContentValues> addHeaders(List<ContentValues> oldList) {
		...
		Calendar lastDate = Calendar.getInstance(); lastDate.setTimeInMillis(0);
	...
	public static boolean match(ContentValues c0, ContentValues c1, boolean filterDuplicates) {
		...
		if (same_account) {
			...
			if (c0.containsKey(DB.FEED.EXTERNAL_ID) && c1.containsKey(DB.FEED.EXTERNAL_ID)) {
				if (c0.getAsString(DB.FEED.EXTERNAL_ID).contentEquals(c1.getAsString(DB.FEED.EXTERNAL_ID)))
					return true;
			}
		...
		for (String k : keys) {
			if (c0.containsKey(k) && c1.containsKey(k))
				if (!c0.getAsString(k).equalsIgnoreCase(c1.getAsString(k))) {
					if (print)
						System.out.println("fail at " + k + " c0: " + c0.getAsString(k) + ", c1: " + c1.getAsString(k));
					return false;
				}
		}
	...
	private static boolean overlaps(ContentValues c0, ContentValues c1, boolean print) {
		...
		boolean broken0 = c0.containsKey(DB.FEED.FLAGS) && c0.getAsString(DB.FEED.FLAGS).contains("brokenStartTime");
		boolean broken1 = c1.containsKey(DB.FEED.FLAGS) && c1.getAsString(DB.FEED.FLAGS).contains("brokenStartTime");
		if (broken0 || broken1) {
			/**
			...
			if (c0.containsKey(DB.FEED.DURATION) && c1.containsKey(DB.FEED.DURATION)) {
				...
				if (dur0 != 0 && dur1 != 0) {
					...
					if (pct > 0.1) {
						if (print)
							System.out.println("fail at dur0: " + dur0 + "dur1: " + dur1 + " => pct: " + pct);
						return false;
					}
					dur_match = 2;
				}
			...
			if (c0.containsKey(DB.FEED.DISTANCE) && c1.containsKey(DB.FEED.DISTANCE)) {
				...
				if (dur0 != 0 && dur1 != 0) {
					...
					if (pct > 0.1) {
						if (print)
							System.out.println("fail at dis0: " + dur0 + "dis1: " + dur1 + " => pct: " + pct);
						return false;
					}
					dis_match = 2;
				}
			...
			System.out.println("dur_match: " + dur_match + ", dis_match: " + dis_match + ", c0: " + FeedList.toString(c0) + ", c1: " + FeedList.toString(c1));
		}
