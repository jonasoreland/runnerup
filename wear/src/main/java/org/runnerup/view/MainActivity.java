...
public class MainActivity extends Activity
  ...
  private class PagerAdapter extends FragmentGridPagerAdapter
    ...
    public Fragment getFragment(int row, int col) {
      if (trackerState.get() == null) return new ConnectToPhoneFragment();
      else if (mStateService == null) return new ConnectToPhoneFragment();
      ...
      switch (trackerState.get()) {
        ...
        case STOPPED:
          if (row == RUN_INFO_ROW) {
            if (pauseStep) {
              if (col == 0) return new CountdownFragment();
    ...
    public int getColumnCount(int i) {
      if (pauseStep) return cols + 1;
      return cols;
    }

    @Override
    public void onValueChanged(
        ValueModel<TrackerState> obj, TrackerState oldValue, TrackerState newValue) {
  ...
  private final ServiceConnection mStateServiceConnection =
  ...
  @Override
  public void onValueChanged(
      ValueModel<TrackerState> obj, final TrackerState oldState, final TrackerState newState) {
    synchronized (trackerState) {
      runOnUiThread(
          new Runnable() {
            @Override
