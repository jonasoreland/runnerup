...
public class MyDotsPageIndicator
  ...
  public void onPageSelected(int row, int col) {
    ...
    if (row < layout.getChildCount()) configDot((Button) layout.getChildAt(row), true);
  }
  ...
  public void onAdapterChanged(
      GridPagerAdapter gridPagerAdapter, GridPagerAdapter gridPagerAdapter2) {}
  ...
  public void onDataSetChanged() {
    ...
    if (pager.getAdapter().getRowCount() <= 1) return;
  ...
  private int getPxFromDp(int dp) {
    return (int)
        TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_DIP, dp, layout.getResources().getDisplayMetrics());
  }
}
